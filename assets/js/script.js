...
var taskFormHandler = function(event) {
    ...
    console.log(isEdit);
    // has data attribute, so get task id and call functio to complete edit process
    if (isEdit) {
        var taskId = formEl.getAttribute("data-task-id");
        completeEditTask(taskNameInput, taskTypeInput, taskId);
    }
    // no data atrtribute, so creat object as normal and pass to createTaskEl function
    else {
        var taskDataObj = {
            name: taskNameInput,
            type: taskTypeInput,
...
var deleteTask = function(taskId) {
    ...
    var updatedTaskArr = [];
    for (let i = 0; i < task.length.length; i++) {
